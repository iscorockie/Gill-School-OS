...
const actions = {
  ...
  addAssessment({ db, payload }) {
    ...
    const assessment = {
      ...
      teacher, date: today(), feedback: feedback || "",
    };
  ...
  updateStudentAccount({ db, payload }) {
    ...
    if (perms) account.perms = { ...account.perms, ...perms };
  ...
  resendFamilyInvite({ db, payload }) {
    ...
    for (const p of parents) {
      db.deliveries.unshift({
        ...
        to: p.phone,
        ref: app.id,
        subject: `Gill School OS access — ${account.inviteLink} · shared login: ${account.username}`,
        status: "delivered",
