...
const actions = {
  ...
  submitApplication({ db, payload }) {
    ...
    const app = db.applications.find((a) => a.id === applicationId);
    if (!app) throw new Error("Application not found.");
    if (app.status !== "in_progress") throw new Error("This application was already submitted.");
    // "review" is completed BY this submission, so it's not a prerequisite.
    const missing = Object.keys(app.steps).filter((k) => k !== "review" && !app.steps[k]);
    ...
    for (const p of parents) {
      db.deliveries.unshift({
        ...
        subject: `Application received — ${kid.name} (${kid.schoolId}) for ${app.intake}. Our Admissions team will verify your documents and confirm by SMS.`,
        status: "delivered", provider: "MTN/Airtel SMS Gateway (simulated)", date: now(),
      });
    }
    db.messages.push({
      id: uid("m"), from: fam.parentUserId, to: "u-admissions",
      subject: `New application received — ${kid.name}`,
      body: `${fam.name} family submitted ${kid.name}'s application (${kid.schoolId}, ${app.campus === "preschool" ? "Pre-School" : "Main School"}, ${kid.class}) for ${app.intake}. ${app.documents.length} document(s) awaiting review. Invoice ${invoice.id} totals UGX ${total.toLocaleString()}.`,
      date: today(), read: false, channel: "email",
    });
    db.feesAudit.unshift({
      id: uid("fa"), date: now(), actor: fam.name,
      action: `Application submitted — ${kid.name} (${kid.schoolId}) · ${app.intake}; invoice ${invoice.id} opened at UGX ${total.toLocaleString()}`,
      amount: total,
    });
